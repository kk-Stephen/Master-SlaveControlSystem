...
plt.title("Trajectories of Bang-Bang leader and its followers")
plt.plot(led_dfs[0]['x'], led_dfs[0]['y'], label='Bang-Bang Leader')
print(led_dfs[0]['x'].iloc[-1])
# for i in range(3):
#     plt.plot(avg_foll_dfs[0][i]['x'], avg_foll_dfs[0][i]['y'], '-.', label="Follower " + sys[0][i])
# plt.plot(avg_foll_dfs[0][0]['x'], avg_foll_dfs[0][0]['y'], '-.', label="BANG-BANG Follower")
plt.plot(avg_foll_dfs[0][1]['x'], avg_foll_dfs[0][1]['y'], '-.', label="PID Follower")
plt.plot(avg_foll_dfs[0][2]['x'], avg_foll_dfs[0][2]['y'], '-.', label="Weighted Follower")
...
# plt.subplot(3, 1, 2)
# plt.title("Trajectories of Weighted leader and its followers")
# plt.plot(led_dfs[1]['x'], led_dfs[1]['y'], label='Weighted Leader')
# print(led_dfs[1]['x'].iloc[-1])
# # for i in range(3):
# #     plt.plot(avg_foll_dfs[1][i]['x'], avg_foll_dfs[1][i]['y'], '-.', label="Follower " + sys[1][i])
# plt.plot(avg_foll_dfs[1][0]['x'], avg_foll_dfs[1][0]['y'], '-.', label="BANG-BANG Follower")
# plt.plot(avg_foll_dfs[1][1]['x'], avg_foll_dfs[1][1]['y'], '-.', label="PID Follower")
# plt.plot(avg_foll_dfs[1][2]['x'], avg_foll_dfs[1][2]['y'], '-.', label="Weighted Follower")
# plt.grid(True, which='both')
# #plt.axhline(y=0, color='k')
# plt.axvline(x=0, color='k')
# plt.xlabel('Horizontal distance (mm)')
...
plt.subplot(2, 1, 2)
plt.title("Trajectories of PID leader and its followers")
plt.plot(led_dfs[2]['x'], led_dfs[2]['y'], label='PID Leader ')
print(led_dfs[2]['x'].iloc[-1])
# for i in range(3):
#     plt.plot(avg_foll_dfs[2][i]['x'], avg_foll_dfs[2][i]['y'], '-.', label="Follower " + sys[2][i])
# plt.plot(avg_foll_dfs[2][0]['x'], avg_foll_dfs[2][0]['y'], '-.', label="BANG-BANG Follower")
plt.plot(avg_foll_dfs[2][1]['x'], avg_foll_dfs[2][1]['y'], '-.', label="PID Follower")
plt.plot(avg_foll_dfs[2][2]['x'], avg_foll_dfs[2][2]['y'], '-.', label="Weighted Follower")
plt.grid(True, which='both')
#plt.axhline(y=0, color='k')
plt.axvline(x=0, color='k')
plt.xlabel('Horizontal distance (mm)')
